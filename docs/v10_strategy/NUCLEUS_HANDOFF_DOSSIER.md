...
## 🚀 5. Next Phase Roadmap (v0.6.0)
1.  **Inject** `DecisionMade` events into the `EphemeralAgent` reasoning loop.
2.  **Bootstrap** the `ContextManager` to generate immutable context hashes.
3.  **Expose** the `/traces` endpoint to allow visual auditing of mission causality.
