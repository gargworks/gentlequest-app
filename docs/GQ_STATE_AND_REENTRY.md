...
| iOS | **1.7.3 (26090302) UPLOADED to App Store Connect 2026-09-03**, Delivery UUID f2b11d5a. Carries welcome + vow instrumentation. Appears in TestFlight after ~10-20 min processing. Path: `scripts/ios_testflight_upload.sh` (no-codesign build + manual signing, codified from STORE_DEPLOYMENT.md steps 4-7). Two traps now handled in-script: `cmd \| grep -q` under pipefail SIGPIPEs a valid check into a failure (bit twice); App.framework ships without `MinimumOSVersion` and Apple rejects with 90360 — injected before signing. Emulator AVD moved to the SSD (`~/.android/avd/Pixel_7.avd` -> symlink), local disk no longer at risk from it. |
