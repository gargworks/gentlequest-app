...
| iOS | **1.7.3 (26090303) uploaded 2026-09-04** (Delivery 05dada15) — a REBUILD: 26090302 launched to a BLACK SCREEN on a real iPhone (iOS 26.1) while 1.7.2 ran fine on the same phone. Cause was PACKAGING, not Flutter. The hand-rolled IPA was zipped with a Python `os.walk` over files only, dropping symlinks — 42 MB where a correct export is 54 MB — and it still passed `codesign --verify --deep --strict` AND Apple ingestion. Also fixed at source: `ios/Flutter/AppFrameworkInfo.plist` lacked `MinimumOSVersion` (Flutter's template has 12.0), the cause of Apple error 90360 and the reason a built framework was being hand-patched. `scripts/ios_testflight_upload.sh` now uses `flutter build ipa` (archive+export) with a size floor. NOTE: STORE_DEPLOYMENT.md's claim that `flutter build ipa` needs an interactive Xcode session is STALE — it builds headless with a manual-signing ExportOptions.plist. | grep -q` under pipefail SIGPIPEs a valid check into a failure (bit twice); App.framework ships without `MinimumOSVersion` and Apple rejects with 90360 — injected before signing. Emulator AVD moved to the SSD (`~/.android/avd/Pixel_7.avd` -> symlink), local disk no longer at risk from it. |
