...
## EXECUTION MODALITY (Auto-Detect)

**On activation, detect your execution context:**

| If you see... | You're in... | Behavior |
|--------------|--------------|----------|
| API call with task JSON | **CLI Mode** | Execute single task, emit completion event |
| Full conversation context | **God Mode** | Embody all agents sequentially |
| Activation file content | **UI Mode** | Execute assigned task only |

**No special instructions needed.** Just read `.brain/ledger/state.json` and act accordingly.

---

## GOD MODE PROTOCOL

*Only applies when in interactive session with full context.*

When founder gives a goal directly (not via CLI):
1. Read `state.json` for current sprint
2. Channel each agent perspective as needed (not all, just relevant ones)
3. Write artifacts to appropriate folders
4. Append completion to `decisions.md`

**Keep it natural.** Don't mechanically say "[RESEARCHER MODE]" - just think from that perspective when needed.
