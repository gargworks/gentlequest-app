...
#   irm https://nucleusos.dev/install.ps1 | iex
#
...
$Repo    = $env:EIDETIC_REPO    ?? 'eidetic-works/nucleus-mcp'
...
Write-Host ""
Write-Host "Pro users: drop sync.json at $env:USERPROFILE\.eidetic\sync.json, then restart the daemon."
Write-Host "  Restore from cloud backup: eideticd --restore"
Write-Host "  Upload immediately:        eideticd --sync-now"
